...
if (strlen($registro) < 8 or strlen($registro) > 9) die ("Quantidade de carateres digitados inválido");
...
if($submit) {
	...
	if(empty($id_estagio)) die ("É obrigatório selecionar a instituicao para o qual está solicitando o termo de compromisso");
	...
	$sql_mural = "select id, id_professor, horario, id_area from mural_estagio where id_estagio=$id_estagio and periodo = '" . PERIODO_ATUAL. "'";
	// echo $sql_mural . "<br>";
	$resultado_mural = $db->Execute($sql_mural);
	$quantidade = $resultado_mural->RecordCount();
	// Logo busco na tabela de estagios
	if ($quantidade == 0) {
		// echo "Campo de estágio não fez selecção neste período<br>";
	...
	if ($aluno == 0) {
		...
		if($resultado_insere === false) die ("0 Nao foi possi­vel inserir o registro na tabela alunos");
    	...
    	$sql_estagiarios .= "values('$id_aluno', '$registro', '$turno', '$nivel', '0', '". date("Y-m-d") ."','$id_estagio', '$id_supervisor', '$id_professor','". PERIODO_ATUAL."','$id_area')";
    	...
    	$sql_estagiarios .= " set turno = '$turno', nivel = '$nivel', tc = '0', tc_solicitacao = '". date("Y-m-d") ."', id_instituicao = '$id_estagio', id_supervisor = '$id_supervisor', id_professor = '$id_professor', periodo = '". PERIODO_ATUAL."', id_area = '$id_area'";
		$sql_estagiarios .= " where id_aluno='$id_aluno' and periodo= '" . PERIODO_ATUAL . "'";
    	$sql_estagiarios . "<br>";
...
while(!$resultado->EOF) {
	...
	if ($quantidade == 0) die("Error: Aluno sem estagios");
	$i = 1;
