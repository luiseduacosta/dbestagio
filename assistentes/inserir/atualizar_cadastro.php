...
$inst_super_id = isset($_POST['inst_super_id']) ? $_POST['inst_super_id'] : NULL;
...
if ((!empty($instituicao_id)) and (!empty($inst_super_id))) {
	$sql_atualiza = "update inst_super set instituicao_id='$instituicao_id'	where id='$inst_super_id'";
...
// die;
$sql = "select id from log_supervisores where id_supervisor = $supervisor_id";
$resultado = $db->Execute($sql);
$quantidade = $resultado->RecordCount();
if ($quantidade > 0) {
    $sql_log = "update log_supervisores set cress='$cress', id_supervisor='$supervisor_id', nome='$nome', ip='$ip', arquivo='$arquivo' where id_supervisor='$supervisor_id'"; 
} else {
    $sql_log = "insert into log_supervisores (cress, id_supervisor, nome, ip, arquivo) values ('$cress', '$supervisor_id', '$nome', '$ip', '$arquivo')";
}
// $sql_log . '<br>';
