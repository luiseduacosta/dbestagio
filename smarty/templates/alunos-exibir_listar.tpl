	...
	<div id='tabela_resumo' style='display:none;'>
		...
		<table border='1' summary='Alunos que cursaram quatro periodos por quantidade de periodos'>
			...
			<tbody>
				...
				<tr>
					<td class='coluna_centralizada'>0</td>
					<td>Todos os períodos no mesmo estágio</td>
					<td class='coluna_centralizada'>{$codigo_0}</td>
				</tr>
				<tr>
					<td class='coluna_centralizada'>1</td>
					<td>Todos os períodos em diferentes estágios</td>
					<td class='coluna_centralizada'>{$codigo_1}</td>
				</tr>
				<tr>
					<td class='coluna_centralizada'>2</td>
					<td>Dois períodos e dois períodos</td>
					<td class='coluna_centralizada'>{$codigo_2}</td>
				</tr>

				<tr>
					<td class='coluna_centralizada'>3</td>
					<td>Dois períodos consecutivos no mesmo estágio (1 e 2)</td>
					<td class='coluna_centralizada'>{$codigo_3}</td>
				</tr>
				<tr>
					<td class='coluna_centralizada'>4</td>
					<td>Dois períodos consecutivos no mesmo estágio (2 e 3)</td>
					<td class='coluna_centralizada'>{$codigo_4}</td>
				</tr>
				<tr>
					<td class='coluna_centralizada'>5</td>
					<td>Dois períodos consecutivos no mesmo estágio (3 e 4)</td>
					<td class='coluna_centralizada'>{$codigo_5}</td>
				</tr>

				<tr>
					<td class='coluna_centralizada'>6</td>
					<td>Três períodos consecutivos no mesmo estágio (1, 2 e 3)</td>
					<td class='coluna_centralizada'>{$codigo_6}</td>
				</tr>
				<tr>
					<td class='coluna_centralizada'>7</td>
					<td>Três períodos consecutivos no mesmo estágio (2, 3 e 4)</td>
					<td class='coluna_centralizada'>{$codigo_7}</td>
				</tr>
				<tr>
					<th class='coluna_centralizada'>TOTAL</th>
					<th>&nbsp;</th>
					<th class='coluna_centralizada'>{$total}</th>
				</tr>
			</tbody>
			...
			<table border="1">
				<tbody>
					<tr>
						...
						<!-- Instituicao //-->
						<td>
							<select name="seleciona_instituicao" size="1" id="instituicao"
								onChange="return get_valor();">
	    ...
	    //-->
						...
						<td>
							<select name="seleciona_periodo" size="1" id="periodo" onChange="return get_valor();">
								...
								<option value="0" selected>Período</option>
								{section name=elemento loop=$matriz_periodo}
								<option value={$matriz_periodo[elemento].turma}>{$matriz_periodo[elemento].turma}
								</option>
								{/section}
								{else}
...
//-->
		...
		{assign var="criterio"
		value="&seleciona_instituicao=$seleciona_instituicao&seleciona_periodo=$seleciona_periodo&seleciona_turno=$seleciona_turno&seleciona_nivel=$seleciona_nivel&seleciona_professor=$seleciona_professor&id_area=$id_area&historia_estilo=$historia_estilo"}
			...
			<table id="alunos" border="1" summary="Lista de alunos">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<a
								href="../atualizar/atualiza_estagio.php?estagiario_id={$lista[i].estagiario_id}&aluno_id={$lista[i].aluno_id}">
								Editar
							</a>
						</td>
						{/if}
						...
						<td style="text-align:center">
							{if $lista[i].tc_solicitacao}
							{$lista[i].tc_solicitacao|date_format:"%d-%m-%Y"}
							{else}
							-
							{/if}
						</td>
						...
						<td class="nome">
							{if $logado == 1}
							<a href="ver_cada.php?aluno_id={$lista[i].aluno_id}">{$lista[i].nome}</a>
							{else}
							{$lista[i].nome}
							{/if}
						</td>
						...
						<td>
							{if $logado == 1}
							<a
								href="../../instituicoes/exibir/ver_cada.php?instituicao_id={$lista[i].instituicao_id}">{$lista[i].instituicao}</a>
							{else}
							{$lista[i].instituicao}
							{/if}
						</td>
						...
						<td>
							{if $logado == 1}
							<a
								href="../../professores/exibir/ver_cada.php?professor_id={$lista[i].professor_id}">{$lista[i].professor}</a>
							{else}
							{$lista[i].professor}
							{/if}
						</td>
			...
			<table id="alunos" border="1" summary="Lista de alunos">
				<thead>
					<tr>
						...
						<th><a href="?ordem=titulo{$criterio}">Título</a></th>
						<th><a href="?ordem=mono_area{$criterio}">Área</a></th>
						<th><a href="?ordem=mono_periodo{$criterio}">Período</a></th>
				...
				<tbody>
					...
					<tr>
						...
						{if $logado == 1}
						<td><a
								href="../atualizar/atualiza_estagio.php?estagiario_id={$lista[i].estagiario_id}&aluno_id={$lista[i].aluno_id}">Editar</a>
						</td>
						{/if}
						...
						<td style="text-align:right">{$lista[i].registro}</td>
						<td style="text-align:center">{$lista[i].tc}</td>
						<td class="nome"><a href="ver_cada.php?aluno_id={$lista[i].aluno_id}">{$lista[i].nome}</a></td>
